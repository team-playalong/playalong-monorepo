...
function TunerCtrl(login, $scope, $timeout, $state, $rootScope) {
  ...
  function updateTuner(noteIndex, noteError, noteFrequency) {
    ...
    let currTick;
    for (let i = 0; i < 8; i++) {
      currTick = document.getElementById('tick_' + i);
      if (currTick) {
        currTick.className = '';
      }
      currTick = document.getElementById('tick_' + (-1) * i);
      if (currTick) {
        currTick.className = '';
      }
    ...
    let leftArrow = document.getElementById('tuneArrowLeft');
    leftArrow && (leftArrow.className = '');
    let rightArrow = document.getElementById('tuneArrowRight');
    rightArrow && (rightArrow.className = '');
    
    let noteView = document.getElementById('noteView');
    noteView && (noteView.innerHTML = notes[noteIndex]);
    
    // Stämd!!
    if (Math.abs(noteError) < 0.05) {
      const tick = document.getElementById('tick_0');
      tick && (tick.className = 'tick_0Highlighted');
      
      // Sätt bokstaven till grön
      const noteView = document.getElementById('noteView');
      noteView && (noteView.className = 'noteCleanHighlighted');
      
      leftArrow && (leftArrow.className = 'tuneArrowLeft_ok');
      rightArrow && (rightArrow.className = 'tuneArrowRight_ok');
    }
    
    else {
      const tick = document.getElementById('tick_0');
      tick && (tick.className = 'tick_0_normal');
      
      noteView && (noteView.className = 'noteWrongHighlighted');
      
      if ((Math.abs(5 * degrees) / 30) < 7.5) {
        const tickToHighlight = document.getElementById('tick_' + Math.round((5 * degrees) / 30));
        tickToHighlight && (tickToHighlight.className = 'tickHighlighted');
      }
      
      if (degrees < 0) {
        leftArrow && (leftArrow.className = 'tuneArrowLeft_wrong');
      }
      
      else {
        rightArrow && (rightArrow.className = 'tuneArrowRight_wrong');
      }
  ...
  function gotStream(stream) {
    ...
    playPause && (playPause.style.display = 'block');
